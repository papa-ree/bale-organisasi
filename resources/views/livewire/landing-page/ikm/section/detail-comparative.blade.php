...
<div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden mb-4">

    <div class="p-4 sm:p-6 border-b border-slate-100 dark:border-slate-700 flex flex-col gap-1">
        <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Analisis Riwayat Detail</p>
        <h3 class="text-base font-black text-slate-800 dark:text-white">Perbandingan 9 Unsur Antar Periode</h3>
        <p class="text-[10px] text-slate-400 dark:text-slate-500">Geser ke samping untuk melihat riwayat lebih lengkap</p>
    </div>
        ...
        <table class="w-full border-collapse">
            <thead>
                <tr class="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700">
                    {{-- Sticky unsur header --}}
                    <th class="sticky left-0 z-10 bg-slate-50 dark:bg-slate-700/50 py-3.5 px-5 text-left text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 border-r border-slate-100 dark:border-slate-700 min-w-[180px]">
                        9 Unsur Pelayanan
                    </th>
                    {{-- Dynamic period columns (newest-first, limited by visibleCount) --}}
                    <template x-for="d in [...visibleChartData].reverse()" :key="d.key">
                        <th class="py-3.5 px-4 text-center min-w-[110px] transition-colors"
                            :class="activePeriod === d.key
                                ? 'bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400'
                                : 'text-slate-500 dark:text-slate-400'">
                            <p class="text-[10px] font-bold uppercase tracking-tight" x-text="d.period.split(' ')[1]"></p>
            ...
            <tbody>
                <template x-for="idx in 9" :key="idx">
                    <tr class="border-b border-slate-100 dark:border-slate-700 last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
                        {{-- Sticky unsur label --}}
                        <td class="sticky left-0 z-10 bg-white dark:bg-slate-800 py-3.5 px-5 border-r border-slate-100 dark:border-slate-700">
                            <div class="flex items-center gap-2.5">
                                <span class="w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-black shrink-0 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 tabular-nums"
                                    x-text="idx"></span>
                                <p class="text-xs font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[130px]"
    ...
    {{-- Legend --}}
    <div class="p-4 bg-slate-50 dark:bg-slate-700/30 border-t border-slate-100 dark:border-slate-700 flex gap-5 flex-wrap">
